...
class DesktopNavigationMixin:
    ...
    def _build_top_bars(self) -> None:
        title_bar = tk.Frame(self.root, bg=PALETTE["title_bar"], height=28)
        title_bar.pack(fill=tk.X)
        title_bar.pack_propagate(False)
        brand = tk.Frame(title_bar, bg=PALETTE["title_bar"])
        brand.pack(side=tk.LEFT, padx=12, pady=4)
        tk.Label(brand, text=f"Electional Software  |  {APP_BUILD_LABEL}", bg=PALETTE["title_bar"], fg="#ffffff", font=("Segoe UI Semibold", 10)).pack(anchor="w")
        top_actions = tk.Frame(title_bar, bg=PALETTE["title_bar"])
        top_actions.pack(side=tk.RIGHT, padx=8, pady=3)
        ...
        self.top_menu_frame = tk.Frame(self.root, bg=PALETTE["top_bar"], padx=10, pady=5)
